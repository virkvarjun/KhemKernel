...
__global__ void layer_norm_kernel(const float* x, const float* gamma, const float* beta, float* out, int M, int N){ 
    int row = blockIdx.x; 
    int tid = threadIdx.x; 
    if (row >= M) return; 
...
int main(){ 
    ...
    CUDA_CHECK(cudaMemcpy(d_g, h_gamma.data(), N * sizeof(float), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_b, h_beta.data(), N * sizeof(float), cudaMemcpyHostToDevice));
