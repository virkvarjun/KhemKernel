...
int main() {
    // Sizes must be multiples of 16 for clean WMMA.
    ...
    std::vector<float> sA(Ms * Ks), sB(Ks * Ns), sC(Ms * Ns), expected(Ms * Ns, 0.0f);
    ...
    matmul_tensor_core(sA.data(), sB.data(), sC.data(), Ms, Ns, Ks);
    float max_err = 0.0f;
    for (int i = 0; i < Ms * Ns; ++i)
        max_err = std::max(max_err, std::abs(sC[i] - expected[i]));
    // Tolerance is much higher because FP16 has ~3 decimal digits of precision.
    printf("matmul_tensor_core: max error = %.4e (FP16 tolerance)\n", max_err);

    // Benchmark.
    matmul_tensor_core(h_A.data(), h_B.data(), h_C.data(), M, N, K);  // warmup

    const int n_runs = 50;
    auto t0 = std::chrono::high_resolution_clock::now();
    for (int i = 0; i < n_runs; ++i) {
        matmul_tensor_core(h_A.data(), h_B.data(), h_C.data(), M, N, K);
    }
    auto t1 = std::chrono::high_resolution_clock::now();
    double ms = std::chrono::duration<double, std::milli>(t1 - t0).count() / n_runs;
    double gflops = (2.0 * M * N * K) / (ms / 1000.0) / 1e9;
    printf("matmul_tensor_core (%dx%dx%d, FP16): %.3f ms, %.1f GFLOPS\n", M, K, N, ms, gflops);
