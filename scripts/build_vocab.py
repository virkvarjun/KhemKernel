...
def build_vocab(token_lists, min_freq=1) -> dict[str, int]: # Build a token-to-index vocabulary from lists of tokens, applying a minimum frequency cutoff
    counter = Counter() # Count how many times each token appears across all token lists
    for tokens in token_lists: # Iterate through each list of tokens and update the counter with the frequency of each token
        counter.update(tokens) # Update the counter with the tokens from the current list
    vocab = list(SPECIAL_TOKENS) # Start the vocabulary with special tokens 
